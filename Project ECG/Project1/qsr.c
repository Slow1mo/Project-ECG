...
int timeCount = 1;												// Number of samples
int SCAN[3] = { 0 };											// Peak finding array
int countDiv = 0;
double Temp = 0;
int count = 0;
...
void peakDetection(QRS_params *params, int x)
{
	...
	//printf(" %7d, %7d, %7.2f, > %7.2f, %7d, %7.2f, < %7.2f, < %7.2f, %7.2f, %7.2f, %7.2f\n", SCAN[1], PEAKS[0], params->THRESHOLD1, params->THRESHOLD2, timeCount, params->RR_LOW, params->RR[1], params->RR_HIGH, params->RR_MISS, params->SPKF, params->NPKF);
	...
	if (PEAKS[0] <= params->THRESHOLD1) {
		...
		if (params->RR[1] > params->RR_LOW && params->RR[1] < params->RR_HIGH) {
			...
			printf("-------------- VALUE OF RPEAK IS; %.2f\n", params->Rpeak);
			printf("-------------- TIME OF RPEAK IS; %.2f\n", params->RR[1]/250);
			printf("-------------- PULSE IS; %d\n", abs(60 / ((params->RR[2] - params->RR[1]) / 250)));
			if (countDiv < 9) {
				countDiv += 1;
			}
	...
	if (params->RR[1] > params->RR_MISS) {
		for (int i = 0; i < sizeof(PEAKS); i++) {
			if (PEAKS[i] > params->THRESHOLD2) {
				...
				// Christian Claudio Sandau code
				int max = params->RecentRR[i] * 5;
				if (max <= params->RecentRR[i + 1]) {
					printf("ATTENTION HIGH LOW MISS HIGH LOW MISS");
				}
